...
export class PaymentComponent implements OnInit {
  ...
  fetchIdCardData(): void {
    ...
    this.http
      .get<any>(environment.api_url + 'getIdCard', {
  ...
  approvePayment(id: string): void {
    this.http
      .post(`${environment.api_url}approvePayment/${id}`, {})
      .subscribe(() => {
  ...
  declinePayment(id: string): void {
    if (confirm('Are you sure you want to delete this ID card request?')) {
      this.http
        .delete(`${environment.api_url}deleteIdCard/${id}`)
        .subscribe(() => {
  ...
  updateCard(): void {
    ...
    this.http
      .put(`${environment.api_url}IDCardUpdate/${id}`, this.editCardData)
      .subscribe(() => {
        this.editMode = false;
        this.fetchIdCardData();
      });
