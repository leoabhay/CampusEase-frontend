...
export class AcademicComponent implements OnInit {
  ...
  updateAcademicRecord(id: string, data: any): Observable<any> {
    return this.http.put(
      `${this.API_URL}updateAcademic/${id}`,
      data,
      this.getAuthHeaders(),
    );
  }
  ...
  deleteAcademicRecord(id: string): Observable<any> {
    return this.http.delete(
      `${this.API_URL}deleteAcademic/${id}`,
      this.getAuthHeaders(),
    );
  }
  ...
  loadStudentAttendance(): void {
    this.http
      .get(`${this.API_URL}getattendancebyemail`, this.getAuthHeaders())
      .subscribe({
        next: (res: any) => {
          console.log('Attendance data:', res);
          // Extract the attendance array inside 'attendance' property
          this.attendanceRecords = Array.isArray(res.attendance)
            ? res.attendance
            : [];
        },
        error: (err) => {
  ...
  loadFaceAttendanceByEmail(): void {
    this.http
      .get(`${this.API_URL}getFaceAttendancesByEmail`, this.getAuthHeaders())
      .subscribe({
        next: (res: any) => {
  ...
  loadStudentAssignments(): void {
    this.http
      .get(`${this.API_URL}getassignmentsbyemail`, this.getAuthHeaders())
      .subscribe({
        next: (res: any) => {
          console.log('Assignment data:', res);
          // Extract the assignments array inside 'Assignment' property
          this.studentAssignments = Array.isArray(res.Assignment)
            ? res.Assignment
            : [];
        },
        error: (err) => {
